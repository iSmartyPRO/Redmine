...
if [ "${OSNAME}" = "debian" ]; then
	...
	pkglist="${pkglist} gcc gcc-c++ wget nginx which"
...
# Create db
echo "CREATE DATABASE redmine DEFAULT CHARACTER SET utf8" | mysql --defaults-file=/root/.my.cnf -N || exit 1
...
echo "GRANT ALL PRIVILEGES ON redmine.* TO 'redmine'@'localhost';" | mysql --defaults-file=/root/.my.cnf -N mysql || exit 1
